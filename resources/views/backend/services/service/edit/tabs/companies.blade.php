<div class="card-content">
    <div class="card-header">
        <h5 class="card-title">
            ...
            <small class="text-muted">@lang('labels.backend.services.service.assign', ['service' => $service->name])</small>
        </h5>
    </div>
    <div class="modal-body">
        {{ html()->form('POST', route('admin.services.service.company.store', [$service->uuid]))->class('form-horizontal')->open() }}

        <div class="row mt-4">
            <div class="col">

                <div class="form-group row">
                    {{ html()->label(__('validation.attributes.backend.services.service.companies'))
                        ...
                        @if($companies->count())
                            @foreach($companies as $company)
                                <div class="checkbox d-flex align-items-center">
                                    {{ html()->label(
                                            html()->checkbox('company_ids[]', $service->companies->contains($company), $company->uuid)
                                                    ->class('switch-input')
